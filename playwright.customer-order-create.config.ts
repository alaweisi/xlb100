...
const customerPort = Number(process.env.CUSTOMER_ORDER_CREATE_QA_APP_PORT ?? "5389");
const customerUrl = `http://127.0.0.1:${customerPort}`;

export default defineConfig({
  testDir: "./tests/e2e/customer-ui",
  testMatch: "customerOrderCreateEvidence.spec.ts",
  outputDir: "./test-results/customer-order-create",
  timeout: 60_000,
  fullyParallel: false,
  workers: 1,
  reporter: [["list"]],
