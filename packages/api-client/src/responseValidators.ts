...
export function validateLoginResponse(value: unknown): LoginResponse | LoginError { const result = object(value, "login response"); if (result.ok === false) { string(result.error, "login response.error"); number(result.statusCode, "login response.statusCode"); if (result.code !== undefined) oneOf(result.code, ["WORKER_ACCESS_SUSPENDED", "WORKER_ACCESS_DISABLED"], "login response.code"); if (result.workerAccessStatus !== undefined) oneOf(result.workerAccessStatus, ["suspended", "disabled"], "login response.workerAccessStatus"); return value as LoginError; } if (result.ok !== true) throw new TypeError("login response.ok must be boolean"); string(result.token, "login response.token"); string(result.userId, "login response.userId"); string(result.role, "login response.role"); return value as LoginResponse; }
