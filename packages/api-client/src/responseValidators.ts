...
export function validateLoginCodeResponse(value: unknown): LoginCodeResponse | LoginError { const result = object(value, "login code response"); if (result.ok === false) { string(result.error, "login code response.error"); number(result.statusCode, "login code response.statusCode"); return value as LoginError; } if (result.ok !== true) throw new TypeError("login code response.ok must be boolean"); string(result.expiresAt, "login code response.expiresAt"); number(result.ttlSeconds, "login code response.ttlSeconds"); number(result.attemptsLeft, "login code response.attemptsLeft"); if (result.stagingDemoCode !== undefined && !/^\d{6}$/u.test(string(result.stagingDemoCode, "login code response.stagingDemoCode"))) throw new TypeError("login code response.stagingDemoCode must contain six digits"); return value as LoginCodeResponse; }
