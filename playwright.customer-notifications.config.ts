...
const customerPort = Number(process.env.CUSTOMER_NOTIFICATIONS_QA_APP_PORT ?? "5393");
const customerUrl = `http://127.0.0.1:${customerPort}`;

export default defineConfig({
  testDir: "./tests/e2e/customer-ui",
  testMatch: "customerNotificationsEvidence.spec.ts",
  outputDir: "./test-results/customer-notifications",
  timeout: 60_000,
  fullyParallel: false,
  workers: 1,
  reporter: [["list"]],
