...
This workstream builds the five XLB application images and freezes registry
...
| `build` | Builds all five images into the local Docker daemon; performs no push and cannot create an image lock. |
| `publish` | Builds and pushes all five images, reads registry digests, generates SBOM/scan evidence, and writes `images.lock.json`. |
