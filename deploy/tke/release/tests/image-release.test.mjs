...
test("local build builds exactly five images and never pushes or freezes a digest", () => {
  const calls = [];
  const result = executeImageRelease({
...
test("authorized publish builds five images, inspects registry digests, and emits valid lock evidence", () => {
  const calls = [];
  const repoRoot = workspace();
  ...
  assert.deepEqual(Object.keys(result.imageLock.images), ["backend", "customer", "worker", "admin", "oa"]);
  assert.equal(result.imageLock.images.worker.digest, digests.worker);
  assert.equal(calls.filter(call => call.includes("--push")).length, 5);
  assert.equal(calls.filter(call => call[0] === "syft").length, 5);
  assert.equal(calls.filter(call => call[0] === "trivy").length, 5);
