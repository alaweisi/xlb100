...
function extractImageContract(values) {
  ...
  if (matches.length < 5) fail("production values must contain five repository/digest pairs");
  const names = ["backend", "customer", "worker", "admin", "oa"];
...
function extractIngressHosts(values) {
  const names = ["api", "customer", "worker", "admin", "oa"];
  const hosts = {};
...
function validateN7Evidence(evidence) {
  ...
  for (const name of ["backend", "customer", "worker", "admin", "oa"]) {
