...
export class SupportQualityService {
    ...
    try {
      return await withTransaction(async (c) => {
        ...
        await eventOutboxRepository.insertEvent(c, {
          ...
          eventType: "support.csat.submitted" as any,
          aggregateType: "support_csat",
          aggregateId: csatId,
          cityCode: i.city as any,
          payload: {
            csatId,
            cityCode: i.city,
            targetType,
  ...
  async createRubric(ctx: RequestContext, input: unknown) {
    const b = body(input),
      ...
      criteria = Array.isArray(b.criteria) ? (b.criteria as any[]) : [];
    if (
      name.length < 1 ||
      name.length > 128 ||
      criteria.length < 1 ||
  ...
  async review(ctx: RequestContext, input: unknown) {
    ...
    return withTransaction(async (c) => {
      ...
      await eventOutboxRepository.insertEvent(c, {
        eventId: generateEventId(),
        eventType: "support.quality.reviewed" as any,
