...
export async function registerGovernanceReadinessRoutes(app: FastifyInstance): Promise<void> {
  app.post("/api/internal/settlement-action-governance/readiness-packets", { preHandler }, async (request: FastifyRequest, reply) => {
    const ctx = getRequestContext(request); const p = createReadinessPacketRequestSchema.safeParse(request.body ?? {});
    if (!p.success) return reply.status(400).send({ ok: false, error: "invalid request" });
    try { const packet = await governanceReadinessService.create(ctx, p.data); return { ok: true, packet }; }
  ...
  app.get("/api/internal/settlement-action-governance/readiness-packets", { preHandler }, async (request, reply) => {
    ...
    try { const packets = await governanceReadinessService.list(ctx, q.intentId); return { ok: true, packets }; }
    catch (e) { return reply.status(500).send({ ok: false, error: String(e) }); }
  });
  app.get<{ Params: { packetId: string } }>("/api/internal/settlement-action-governance/readiness-packets/:packetId", { preHandler }, async (request, reply) => {
    ...
    try { const p = await governanceReadinessService.get(ctx, request.params.packetId); if (!p) return reply.status(404).send({ ok: false, error: "not found" }); return { ok: true, packet: p }; }
    catch (e) { return reply.status(500).send({ ok: false, error: String(e) }); }
  });
  app.post<{ Params: { packetId: string } }>("/api/internal/settlement-action-governance/readiness-packets/:packetId/recompute-checks", { preHandler }, async (request, reply) => {
    const ctx = getRequestContext(request);
    try { const p = await governanceReadinessService.recomputeChecks(ctx, request.params.packetId); if (!p) return reply.status(404).send({ ok: false, error: "not found" }); return { ok: true, packet: p }; }
    catch (e) { return reply.status(500).send({ ok: false, error: String(e) }); }
  });
  app.post<{ Params: { packetId: string } }>("/api/internal/settlement-action-governance/readiness-packets/:packetId/mark-blocked", { preHandler }, async (request, reply) => {
    const ctx = getRequestContext(request);
    try { const p = await governanceReadinessService.markBlocked(ctx, request.params.packetId); if (!p) return reply.status(404).send({ ok: false, error: "not found" }); return { ok: true, packet: p }; }
    catch (e) { return reply.status(500).send({ ok: false, error: String(e) }); }
  });
  app.post<{ Params: { packetId: string } }>("/api/internal/settlement-action-governance/readiness-packets/:packetId/archive", { preHandler }, async (request, reply) => {
    const ctx = getRequestContext(request);
    try { const p = await governanceReadinessService.archive(ctx, request.params.packetId); if (!p) return reply.status(404).send({ ok: false, error: "not found" }); return { ok: true, packet: p }; }
    catch (e) { return reply.status(500).send({ ok: false, error: String(e) }); }
  });
}
