...
export function CustomerOrdersPage({ api, cityCode, orderIds }: CustomerOrdersPageProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (cancelled) return;
      const loadedOrders = results.flatMap((result) => result.status === "fulfilled" ? [result.value.order] : []);
      const failures = results.flatMap((result, index) => result.status === "rejected" ? [orderIds[index]] : []);
      setOrders(loadedOrders);
      setFailedOrderIds(failures.filter((id): id is string => Boolean(id)));
  ...
  async function payAfterService(orderId: string) {
    ...
    try {
      ...
      await api.mockPaySuccess({
        paymentOrderId: payment.paymentOrder.paymentOrderId,
        providerTradeNo: `customer-service-${Date.now()}`,
        status: "paid",
      });
      ...
      setPaymentStates((previous) => ({ ...previous, [orderId]: "success" }));
      setPaymentMessages((previous) => ({ ...previous, [orderId]: "支付成功，订单已完成。" }));
    } catch (cause) {
