...
/** @deprecated Use loginCustomer() + session.userId instead. Kept for backward compatibility. */
...
export function readStoredSession(): CustomerSession | null {
  ...
  const userId = window.localStorage.getItem(CUSTOMER_USER_ID_KEY) ?? "";
  return { token, userId };
}
...
export function clearStoredSession(): void {
  ...
  window.localStorage.removeItem(CUSTOMER_USER_ID_KEY);
}

function storeSession(session: CustomerSession): void {
  if (typeof window === "undefined") return;
  window.localStorage.setItem(TOKEN_STORAGE_KEY, session.token);
  window.localStorage.setItem(CUSTOMER_USER_ID_KEY, session.userId);
}
...
export async function loginCustomer(phone = "13800000001"): Promise<CustomerSession> {
  const authApi = createAuthApi(createApiClient({ baseUrl: "" }));
  const codeRequest = await authApi.requestCustomerLoginCode(phone);
  if (!codeRequest.ok) {
    throw new CustomerAuthError("Login code request failed", codeRequest.statusCode);
  }
  const debugCode = await authApi.getCustomerDebugCode(phone);
  if (!debugCode.ok) {
    throw new CustomerAuthError("Login debug code unavailable", debugCode.statusCode);
  }
  const result = await authApi.customerLogin(phone, debugCode.code);
  if (!result.ok) {
    throw new CustomerAuthError("Login failed", result.statusCode);
  }
...
export function createCustomerApiClient(cityCode: CityCode, token?: string) {
  ...
  return customerApi.forClient(createApiClient({ baseUrl: "", headers }));
}
