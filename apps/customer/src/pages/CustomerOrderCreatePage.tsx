...
export function CustomerOrderCreatePage({
  ...
  useEffect(() => {
    ...
    void api.getPriceQuote(selectedSkuId)
      ...
      .catch(() => {
        setQuoteState({ status: "error", error: "当前报价可能已更新，请重新获取后再确认预约。" });
      });
  }, [api, selectedSkuId]);
  ...
  async function submitOrder() {
    if (!canSubmit || !selectedSkuId) {
      setSubmitState({ status: "error", error: "请完成服务、地址、联系人和预约时间后再提交" });
      return;
    }
    ...
    try {
      ...
      setSubmitState({ status: "error", error: error instanceof Error ? error.message : "提交失败，请重试" });
    }
  }
        ...
        {catalogState.status === "error" ? (
          <div className="order-create-state">
            <ErrorState
              ...
              action={onRetryCatalog ? <Button onClick={onRetryCatalog}>重新加载</Button> : undefined}
            />
          </div>
        ) : null}
    ...
    stepContent = (
      <section className="order-create-step" data-step="confirm">
        ...
        <section className="order-create-quote" aria-label="服务报价">
          ...
          {quoteState.status === "error" ? (
            <ErrorState title="报价获取失败" description={quoteState.error} />
          ) : null}
          {quoteState.status === "success" ? (
            <div className="order-create-quote-value">
              <span><strong>{quoteState.quote.priceText}</strong><small>{quoteState.quote.priceType}</small></span>
              <PriceText amount={quoteState.quote.basePrice} currency={quoteState.quote.currency} />
            </div>
          ) : null}
        </section>
        ...
        {submitState.status === "error" ? (
          <div className="order-create-submit-error" role="alert"><WarningCircle aria-hidden="true" size={20} /><span>{submitState.error}</span></div>
        ) : null}
      </section>
    );
  }
  ...
  const successContent = submitState.status === "success" ? (
    <section className="order-create-success" aria-live="polite">
      ...
      <dl>
        ...
        <div><dt>订单状态</dt><dd><StatusTag tone={statusTone(submitState.orderDetail.status)}>{orderStatusLabels[submitState.orderDetail.status] ?? submitState.orderDetail.status}</StatusTag></dd></div>
      </dl>
  ...
  return (
    <CustomerRouteShell
      ...
      topBar={<BookingTopBar step={submitState.status === "success" ? 4 : activeStep} />}
    >
