  ...
  assert.throws(
    () =>
      resolveMobileEnvironment(app, "test", {
        XLB_ADMIN_MOBILE_API_BASE_URL: "http://example.com",
      }),
    /pinned/u,
  );
});

test("development and production require an explicit HTTPS origin", () => {
  for (const profile of ["development", "production"]) {
    assert.equal(
      resolveMobileEnvironment(app, profile, {
        XLB_ADMIN_MOBILE_API_BASE_URL: "https://api.example.com",
      }).apiBaseUrl,
    ...
    assert.throws(
      () =>
        resolveMobileEnvironment(app, profile, {
          XLB_ADMIN_MOBILE_API_BASE_URL: "http://api.example.com",
        }),
      /must use HTTPS/u,
    );
  }
  ...
  assert.throws(
    () =>
      resolveMobileEnvironment(app, "production", {
        XLB_ADMIN_MOBILE_API_BASE_URL: "https://api.example.com/api",
      }),
    /must be an origin/u,
  );
  assert.throws(
    () =>
      resolveMobileEnvironment(app, "production", {
        XLB_ADMIN_MOBILE_API_BASE_URL: "https://user:pass@api.example.com",
      }),
    /must be an origin/u,
  );
});

test("mobile web build reuses Admin source and injects its existing API variable", () => {
  ...
  assert.deepEqual(plan.webBuild.args.slice(-3), [
    "--outDir",
