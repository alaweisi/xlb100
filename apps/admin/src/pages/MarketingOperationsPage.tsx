  ...
  return (
    <main className="marketing-operations" aria-labelledby="marketing-operations-title">
      ...
      <div className="marketing-operations__toolbar">
        ...
        <button type="button" onClick={() => void load()} disabled={!online || state === "loading"}>刷新</button>
      ...
      {state === "loading" && <p role="status">正在读取营销数据…</p>}
        ...
        <ul className="marketing-operations__list">
          {campaigns.map((item) => <li className="marketing-operations__row" key={item.marketingCampaignId}>
            ...
            {canAdminMutate && <div className="marketing-operations__actions">
              {item.status === "draft" && <button type="button" disabled={!online || busyId !== null} onClick={() => void run(item.marketingCampaignId, () => api.reviewCampaign(item.marketingCampaignId, { expectedVersion: item.version, reason }))}>审核</button>}
              ...
              {(item.status === "scheduled" || item.status === "paused") && <button type="button" disabled={!online || busyId !== null} onClick={() => void run(item.marketingCampaignId, () => api.changeCampaignStatus(item.marketingCampaignId, { status: "active", expectedVersion: item.version, reason }))}>启用</button>}
              {item.status === "active" && <button type="button" disabled={!online || busyId !== null} onClick={() => void run(item.marketingCampaignId, () => api.changeCampaignStatus(item.marketingCampaignId, { status: "paused", expectedVersion: item.version, reason }))}>暂停</button>}
              {(item.status === "active" || item.status === "paused") && <button type="button" disabled={!online || busyId !== null} onClick={() => void run(item.marketingCampaignId, () => api.changeCampaignStatus(item.marketingCampaignId, { status: "ended", expectedVersion: item.version, reason }))}>结束</button>}
            </div>}
          </li>)}
        </ul>
      </section>}
      ...
      {tab === "rules" && <section className="marketing-operations__panel" aria-label="规则修订管理">
        <div className="marketing-operations__actions">
          ...
          <button type="button" onClick={() => setRuleCampaignId(ruleCampaignInput.trim())} disabled={!online || !ruleCampaignInput.trim() || state === "loading"}>读取规则</button>
        </div>
        ...
        <ul className="marketing-operations__list">{ruleRevisions.map((item) => <li className="marketing-operations__row" key={item.ruleRevisionId}>
          ...
          {canAdminMutate && <div className="marketing-operations__actions">
            {item.status === "draft" && <button type="button" disabled={!online || busyId !== null} onClick={() => void run(item.ruleRevisionId, () => api.reviewRuleRevision(item.ruleRevisionId, { expectedVersion: item.version, reason }))}>审核规则</button>}
            {item.status === "reviewed" && <button type="button" disabled={!online || busyId !== null} onClick={() => void run(item.ruleRevisionId, () => api.publishRuleRevision(item.ruleRevisionId, { expectedVersion: item.version, reason }))}>发布规则</button>}
          </div>}
        </li>)}</ul>
      </section>}
      ...
      {tab === "definitions" && <section className="marketing-operations__panel" aria-label="券定义管理">
        ...
        <ul className="marketing-operations__list">{definitions.map((item) => <li className="marketing-operations__row" key={item.couponDefinitionId}>
          ...
          {canAdminMutate && <div className="marketing-operations__actions">
            {item.status === "draft" && <button type="button" disabled={!online || busyId !== null} onClick={() => void run(item.couponDefinitionId, () => api.changeCouponDefinitionStatus(item.couponDefinitionId, { status: "active", expectedVersion: item.version, reason }))}>启用</button>}
            {item.status === "active" && <button type="button" disabled={!online || busyId !== null} onClick={() => void run(item.couponDefinitionId, () => api.changeCouponDefinitionStatus(item.couponDefinitionId, { status: "suspended", expectedVersion: item.version, reason }))}>暂停</button>}
      ...
      {tab === "grants" && <section className="marketing-operations__panel" aria-label="优惠券发放">
        ...
        {state === "success" && grants.length === 0 && <p>本次会话尚未发放优惠券；当前契约未开放后台发放列表接口。</p>}
        <ul className="marketing-operations__list">{grants.map((item) => <li className="marketing-operations__row" key={item.couponGrantId}>
          ...
          {canAdminMutate && ["granted", "available", "released"].includes(item.status) && <button type="button" disabled={!online || busyId !== null} onClick={() => void run(item.couponGrantId, () => api.revokeCouponGrant(item.couponGrantId, { expectedVersion: item.version, reason }))}>撤销</button>}
        </li>)}</ul>
      </section>}
    </main>
  );
