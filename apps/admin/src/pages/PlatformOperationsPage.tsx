  ...
  return <div style={{display:"grid",gap:16}}>
    ...
    <Card title="SKU Availability" actions={<><StatusTag tone="primary">{skus.length}</StatusTag><StatusTag tone="warning">canonical catalog write</StatusTag></>}>
      {skus.length===0?<EmptyState title="No city SKU"/>:<Table rows={skus} getRowKey={row=>row.skuId} columns={[
        ...
        {key:"control",title:"Control",render:row=><ConfirmButton disabled={busy!==null} onConfirm={()=>void act(`sku:${row.skuId}`,()=>api.setOperationsSkuEnabled(row.skuId,!row.isEnabled))}>{row.isEnabled?"Disable":"Enable"}</ConfirmButton>},
      ]}/>}
    </Card>
    <Card title="Worker Certification Review" actions={<StatusTag tone="primary">{certifications.length}</StatusTag>}>
      {certifications.length===0?<EmptyState title="No certification application"/>:<Table rows={certifications} getRowKey={row=>row.certificationId} columns={[
        ...
        {key:"review",title:"Review",render:row=><div style={{display:"flex",gap:8}}><ConfirmButton disabled={busy!==null||row.status!=="pending"} onConfirm={()=>void act(`approve:${row.certificationId}`,()=>api.approveWorkerCertification(row.certificationId))}>Approve</ConfirmButton><ConfirmButton disabled={busy!==null||row.status!=="pending"} onConfirm={()=>void act(`reject:${row.certificationId}`,()=>api.rejectWorkerCertification(row.certificationId,"Requirements not met"))}>Reject</ConfirmButton></div>},
      ]}/>}
    </Card>
  </div>;
}
