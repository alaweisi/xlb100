  ...
  return <>
    ...
    <Card title="联系平台客服" actions={<StatusTag tone={networkOnline ? "success" : "danger"}>{networkOnline ? "已连接" : "已离线"}</StatusTag>} style={workerPanelStyle}>
      <div className="worker-stack-list">
        ...
        <Button variant="primary" disabled={!networkOnline || busy !== null || subject.trim().length < 3 || description.trim().length < 5} onClick={() => void create()}>{uiChoice(uiStateIs(busy, "create"), "正在提交", "提交客服工单")}</Button>
    ...
    <Card title="在线客服会话" actions={<div className="worker-card-actions"><Button disabled={!networkOnline || busy !== null} onClick={() => void loadConversations()}>刷新会话</Button><Button disabled={!networkOnline || busy !== null} variant="primary" onClick={() => void startConversation()}>发起会话</Button></div>} style={workerPanelStyle}>
      {conversations.length === 0 ? <EmptyState title="暂无已加载会话" description="点击刷新读取历史会话，或发起新的客服会话。" /> : <div className="worker-card-actions">{conversations.map((item) => <Button key={item.conversationId} onClick={() => void openConversation(item.conversationId)}>{uiChoice(uiStateIs(item.status, "closed"), "已关闭", "处理中")} · {formatDateTime(item.updatedAt)}</Button>)}</div>}
      {conversation && <div className="worker-stack-list">{conversation.messages.map((message) => <div key={message.messageId} style={mutedBoxStyle}><strong>{uiChoice(uiStateIs(message.senderType, "worker"), "我", "客服")}</strong><span style={helperText}>{message.textContent || "图片消息"}</span></div>)}<Textarea aria-label="会话消息" value={chatText} onChange={(e) => setChatText(e.target.value)} /><Button variant="primary" disabled={!networkOnline || busy !== null || !chatText.trim() || uiStateIs(conversation.conversation.status, "closed")} onClick={() => void sendChat()}>{uiChoice(uiStateIs(busy, "chat"), "正在发送", "发送消息")}</Button></div>}
    </Card>

    <Card title="我的客服工单" actions={<Button disabled={!networkOnline || busy !== null} onClick={() => void load()}>刷新工单</Button>} style={workerPanelStyle}>
      {uiStateIs(busy, "list") && tickets.length === 0 ? <LoadingState title="正在加载工单" /> : tickets.length === 0 ? <EmptyState title="暂无客服工单" /> : <Table rows={tickets} getRowKey={(row) => row.ticketId} columns={[{ key: "subject", title: "问题", render: (row) => row.subject }, { key: "status", title: "状态", render: (row) => ticketStatus[row.status] ?? "未知状态" }, { key: "open", title: "", render: (row) => <Button disabled={busy !== null} onClick={() => void open(row.ticketId)}>查看</Button> }]} />}
    </Card>

    {detail && <Card title={`工单 · ${detail.ticket.subject}`} actions={<StatusTag tone={statusTone(detail.ticket.status)}>{ticketStatus[detail.ticket.status] ?? "未知状态"}</StatusTag>} style={workerPanelStyle}><div className="worker-stack-list"><p style={helperText}>{detail.ticket.description}</p>{detail.ticket.linkedAftersaleComplaintId && <div style={mutedBoxStyle}><strong>关联售后投诉：{detail.ticket.linkedAftersaleComplaintId}</strong><span style={helperText}>此处仅供查看；售后操作仍需在售后返工流程中完成。</span></div>}{detail.events.map((event) => <div key={event.ticketEventId} style={mutedBoxStyle}><strong>{eventLabels[event.eventType] ?? "工单状态更新"}</strong><span style={helperText}>{event.content || "状态已更新"} · {formatDateTime(event.createdAt)}</span></div>)}<FormField label="补充留言"><Textarea value={comment} onChange={(e) => setComment(e.target.value)} /></FormField><div className="worker-card-actions"><Button variant="primary" disabled={!networkOnline || busy !== null || !comment.trim()} onClick={() => void send()}>{uiChoice(uiStateIs(busy, "comment"), "正在发送", "发送留言")}</Button><Button disabled={!networkOnline || busy !== null || detail.ticket.status !== "resolved"} onClick={() => void reopen()}>重新打开</Button>{uiStateIs(detail.ticket.status, "closed") && <Button disabled={!networkOnline || busy !== null} onClick={() => void api.submitCsat(detail.ticket.ticketId, { score: 5, idempotencyKey: requestKey("worker-csat") })}>评价客服 5 分</Button>}</div></div></Card>}
  </>;
}
