...
import { formatDateTime, fulfillmentStatusLabel, helperText, statusTone, uiChoice, uiStateIn, uiStateIs, workerPanelStyle } from "./pageShared";
    ...
    <>
      ...
      <section className="worker-journey-section" aria-labelledby="repair-title">
        ...
        {repairOrders.length === 0 && !loading ? <Card style={workerPanelStyle}><EmptyState title="暂无返工任务" description="平台分派的投诉返工任务会显示在这里。" /></Card> : <div className="worker-task-list">{repairOrders.map((item) => <article className={`worker-task-card worker-task-card--${item.status}`} key={item.repairOrderId}>
          ...
          <ol className="worker-timeline"><li className={`worker-timeline__item worker-timeline__item--${uiChoice(uiStateIs(item.status, "requested"), "current", "done")}`}><span aria-hidden="true"/><div><strong>返工申请</strong><small>{formatDateTime(item.createdAt)}</small></div></li><li className={`worker-timeline__item worker-timeline__item--${uiChoice(uiStateIs(item.status, "assigned"), "current", uiChoice(uiStateIn(item.status, ["in_progress", "completed"]), "done", uiChoice(uiStateIs(item.status, "cancelled"), "blocked", "pending")))}`}><span aria-hidden="true"/><div><strong>平台分派</strong><small>{item.workerId ? `已分派师傅 ${item.workerId}` : "等待分派"}</small></div></li><li className={`worker-timeline__item worker-timeline__item--${uiChoice(uiStateIs(item.status, "in_progress"), "current", uiChoice(uiStateIs(item.status, "completed"), "done", uiChoice(uiStateIs(item.status, "cancelled"), "blocked", "pending")))}`}><span aria-hidden="true"/><div><strong>现场返工</strong><small>{item.startedAt ? formatDateTime(item.startedAt) : "尚未开始"}</small></div></li><li className={`worker-timeline__item worker-timeline__item--${uiChoice(uiStateIs(item.status, "completed"), "done", uiChoice(uiStateIs(item.status, "cancelled"), "blocked", "pending"))}`}><span aria-hidden="true"/><div><strong>{uiChoice(uiStateIs(item.status, "cancelled"), "任务取消", "登记完成")}</strong><small>{item.completedAt ? formatDateTime(item.completedAt) : uiChoice(uiStateIs(item.status, "cancelled"), "平台已取消", "尚未完成")}</small></div></li></ol>
