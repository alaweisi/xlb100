...
import { formatBusinessCode, formatCityName, formatDateTime, formatServiceName, fulfillmentStatusLabel, helperText, statusTone, uiChoice, uiStateIn, uiStateIs, workerPanelStyle } from "./pageShared";
    ...
    <>
      ...
      <section className="worker-journey-section" aria-labelledby="repair-title">
        ...
        {repairOrders.length === 0 && !loading ? <Card style={workerPanelStyle}><EmptyState title="暂无返工任务" description="平台分派的投诉返工任务会显示在这里。" /></Card> : <div className="worker-task-list">{repairOrders.map((item) => <article className={`worker-task-card worker-task-card--${item.status}`} key={item.repairOrderId}>
          <div className="worker-task-card__topline"><div><span>返工编号</span><strong>{formatBusinessCode(item.repairOrderId, "返工单")}</strong></div><StatusTag tone={statusTone(item.status)}>{repairStatusLabels[item.status]}</StatusTag></div>
          <p className="worker-primary-guidance">{uiChoice(uiStateIs(item.status, "requested"), "返工申请已创建，等待平台分派；当前无需操作。", uiChoice(uiStateIs(item.status, "assigned"), "平台已分派给你，请核对订单和原因后开始返工。", uiChoice(uiStateIs(item.status, "in_progress"), "返工处理中，请完成现场服务并如实填写结果。", uiChoice(uiStateIs(item.status, "completed"), "返工已登记完成，请保留现场记录并关注后续售后结果。", "该返工已取消，操作入口已关闭。"))))}</p>
          <ol className="worker-timeline"><li className={`worker-timeline__item worker-timeline__item--${uiChoice(uiStateIs(item.status, "requested"), "current", "done")}`}><span aria-hidden="true"/><div><strong>返工申请</strong><small>{formatDateTime(item.createdAt)}</small></div></li><li className={`worker-timeline__item worker-timeline__item--${uiChoice(uiStateIs(item.status, "assigned"), "current", uiChoice(uiStateIn(item.status, ["in_progress", "completed"]), "done", uiChoice(uiStateIs(item.status, "cancelled"), "blocked", "pending")))}`}><span aria-hidden="true"/><div><strong>平台分派</strong><small>{item.workerId ? `已分派${formatBusinessCode(item.workerId, "师傅编号")}` : "等待分派"}</small></div></li><li className={`worker-timeline__item worker-timeline__item--${uiChoice(uiStateIs(item.status, "in_progress"), "current", uiChoice(uiStateIs(item.status, "completed"), "done", uiChoice(uiStateIs(item.status, "cancelled"), "blocked", "pending")))}`}><span aria-hidden="true"/><div><strong>现场返工</strong><small>{item.startedAt ? formatDateTime(item.startedAt) : "尚未开始"}</small></div></li><li className={`worker-timeline__item worker-timeline__item--${uiChoice(uiStateIs(item.status, "completed"), "done", uiChoice(uiStateIs(item.status, "cancelled"), "blocked", "pending"))}`}><span aria-hidden="true"/><div><strong>{uiChoice(uiStateIs(item.status, "cancelled"), "任务取消", "登记完成")}</strong><small>{item.completedAt ? formatDateTime(item.completedAt) : uiChoice(uiStateIs(item.status, "cancelled"), "平台已取消", "尚未完成")}</small></div></li></ol>
          <dl className="worker-fact-grid"><div><dt>订单编号</dt><dd>{formatBusinessCode(item.orderId, "订单")}</dd></div><div><dt>投诉编号</dt><dd>{formatBusinessCode(item.complaintId, "投诉")}</dd></div><div><dt>返工原因</dt><dd>{item.reason}</dd></div>{item.serviceNote && <div><dt>完成说明</dt><dd>{item.serviceNote}</dd></div>}</dl>
          ...
          <div className="worker-card-actions"><Button disabled={!networkOnline || item.status !== "assigned" || busyId === item.repairOrderId} onClick={() => onStart(item.repairOrderId)}>{uiChoice(busyId === item.repairOrderId && uiStateIs(item.status, "assigned"), "正在开始", "开始返工")}</Button><Button variant="primary" disabled={!networkOnline || item.status !== "in_progress" || busyId === item.repairOrderId || !(notes[item.repairOrderId] ?? "").trim()} onClick={() => onComplete(item.repairOrderId, (notes[item.repairOrderId] ?? "").trim())}>{uiChoice(busyId === item.repairOrderId && uiStateIs(item.status, "in_progress"), "正在登记", "登记返工完成")}</Button></div>
          {uiStateIn(item.status, ["requested", "cancelled"]) && <p className="worker-contract-note">师傅侧现有接口不提供接收、拒绝或取消返工操作，本页只展示平台真实状态，不伪造操作结果。</p>}
        </article>)}</div>}
      </section>
    </>
  );
...
export function TaskDetailPage({
  ...
  return (
    <>
      <div className="worker-detail-toolbar"><Button onClick={onBack}>返回任务列表</Button><span>{formatBusinessCode(fulfillmentId, "履约单")}</span></div>
      ...
      {!loading && !error && fulfillment && <>
        <Card title="当前履约状态" actions={<StatusTag tone={statusTone(fulfillment.status)}>{fulfillmentStatusLabel(fulfillment.status)}</StatusTag>} style={workerPanelStyle}>
          <p className="worker-primary-guidance">{uiChoice(uiStateIs(fulfillment.status, "accepted"), "到达服务现场后开始服务，并及时留存过程证据。", uiChoice(uiStateIs(fulfillment.status, "in_progress"), "服务正在进行，请补齐关键证据后登记完工。", uiChoice(uiStateIs(fulfillment.status, "completed"), "服务已完工，请关注顾客确认或争议结果。", "该任务已取消，履约操作已关闭。")))}</p>
        ...
        <Card title="任务信息" style={workerPanelStyle}><dl className="worker-fact-grid worker-fact-grid--detail"><div><dt>订单编号</dt><dd>{formatBusinessCode(fulfillment.orderId, "订单")}</dd></div><div><dt>服务项目</dt><dd>{formatServiceName(fulfillment.skuId)}</dd></div><div><dt>工作城市</dt><dd>{formatCityName(fulfillment.cityCode)}</dd></div><div><dt>派单编号</dt><dd>{formatBusinessCode(fulfillment.dispatchTaskId, "派单")}</dd></div><div><dt>开始时间</dt><dd>{formatDateTime(fulfillment.startedAt)}</dd></div><div><dt>完工时间</dt><dd>{formatDateTime(fulfillment.completedAt)}</dd></div></dl>{fulfillment.completionNote && <div className="worker-completion-note"><span>完工说明</span><p>{fulfillment.completionNote}</p></div>}</Card>
        ...
        <Card title="履约操作" actions={<StatusTag tone="primary">真实状态机</StatusTag>} style={workerPanelStyle}>
          <div className="worker-lifecycle-actions">
            <div><strong>1. 开始服务</strong><span>{uiChoice(uiStateIs(fulfillment.status, "accepted"), "确认到场后开始计入履约过程。", "仅待开始状态可操作。")}</span><Button disabled={!canStart} onClick={() => onStart(fulfillmentId)} variant="primary">{uiChoice(uiStateIs(lifecycleAction, "start"), "正在确认开始", "开始服务")}</Button></div>
            <div><strong>2. 登记完工</strong><span>{uiChoice(uiStateIs(fulfillment.status, "in_progress"), "填写实际结果；完工后等待顾客确认。", "仅服务中状态可操作。")}</span><FormField label="完工说明（选填，最多 255 字）"><Textarea disabled={!canComplete} maxLength={255} value={completionNote} onChange={(event) => setCompletionNote(event.target.value)} /></FormField><Button disabled={!canComplete} onClick={() => onComplete(fulfillmentId, completionNote.trim() || undefined)} variant="primary">{uiChoice(uiStateIs(lifecycleAction, "complete"), "正在确认完工", "登记完工")}</Button></div>
          </div>
        </Card>
        ...
        {!evidenceLoading && evidenceAggregate && <Card title="证据时间线" style={workerPanelStyle}>{evidenceAggregate.evidence.length === 0 ? <EmptyState title="尚未上传证据" description="建议按到场、服务前后和完工节点留存现场图片。" /> : <div className="worker-evidence-list">{evidenceAggregate.evidence.map((item) => <article key={item.evidenceId}><div><StatusTag tone="primary">{evidenceLabels[item.evidenceType]}</StatusTag><strong>{formatBusinessCode(item.mediaAsset.originalFileName, "证据图片")}</strong></div><span>{formatDateTime(item.capturedAt)} · {Math.max(1, Math.round(item.mediaAsset.sizeBytes / 1024))} 千字节</span><small>完整性校验已记录 · {uiChoice(uiStateIs(item.mediaAsset.securityScanStatus, "not_malware_scanned_local"), "本地格式检查", "安全检查状态未知")}</small></article>)}</div>}</Card>}

        <Card title="顾客确认与争议" actions={<StatusTag tone={uiChoice(uiStateIs(confirmationStatus, "confirmed"), "success", uiChoice(uiStateIs(confirmationStatus, "disputed"), "danger", "warning"))}>{uiChoice(uiStateIs(confirmationStatus, "confirmed"), "已确认", uiChoice(uiStateIs(confirmationStatus, "disputed"), "有争议", "待确认"))}</StatusTag>} style={workerPanelStyle}><ConfirmationPanel aggregate={evidenceAggregate} completed={fulfillment.status === "completed"} /></Card>
