...
const customerPort = Number(process.env.CUSTOMER_COUPONS_QA_APP_PORT ?? "5387");
const customerUrl = `http://127.0.0.1:${customerPort}`;

export default defineConfig({
  testDir: "./tests/e2e/customer-ui",
  testMatch: "customerCouponsEvidence.spec.ts",
  outputDir: "./test-results/customer-coupons",
  timeout: 60_000,
  fullyParallel: false,
  workers: 1,
  reporter: [["list"]],
