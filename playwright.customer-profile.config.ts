...
const customerPort = Number(process.env.CUSTOMER_PROFILE_QA_APP_PORT ?? "5395");
const customerUrl = `http://127.0.0.1:${customerPort}`;

export default defineConfig({
  testDir: "./tests/e2e/customer-ui",
  testMatch: "customerProfileEvidence.spec.ts",
  outputDir: "./test-results/customer-profile",
  timeout: 60_000,
  fullyParallel: false,
  workers: 1,
  reporter: [["list"]],
