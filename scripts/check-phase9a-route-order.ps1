...
$violations = $diff | Select-String "backend/src/" | Where-Object { $_ -notmatch 'backend/src/governance/' -and $_ -notmatch 'backend/src/app\.ts' }
if ($violations) { Write-Host "check-phase9a-route-order: FAILED — $violations"; exit 1 }
