...
$Forbidden = @(
  ...
  'UPDATE settlement_batches[\s\S]*status', 'UPDATE worker_receivable_statement_lines'
)
foreach ($pattern in $Forbidden) {
  if ($Service -match $pattern) { throw "Phase 8G must not mutate upstream or settlement status: $pattern" }
}
Write-Host "PASS: worker receivable statement reviews do not mutate upstream state."
