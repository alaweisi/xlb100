...
try {
  ...
  foreach ($appName in @("customer", "worker", "admin")) {
    ...
    if ($appName -eq "customer") {
      ...
      $homeTemplate = Get-Content (Join-Path $Root "apps/customer/src/features/home/CustomerSduiPageTemplate.tsx") -Raw
      ...
      Require-Match "customer registered route runtime" $app '(?is)<CustomerAppRouter'
      Require-Match "customer dynamic HomePage entry" $homeTemplate '(?is)<HomePage\s+telemetry='
    } else {
