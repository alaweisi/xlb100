...
const b0=model.slices.filter(x=>x.batch==='B0');const baseIds=['C-00','W-00','A-00'];const metrics=[['正式切片',model.metrics.sliceCount+'/214'],['B0 Edge 已验',b0.filter(x=>x.status==='EDGE_VERIFIED').length+'/7'],['三端基础外壳',model.carriers.filter(x=>baseIds.includes(x.carrierId)&&x.baseFrame.status==='EDGE_VERIFIED').length+'/3'],['最终人工验收',model.metrics.acceptedCount+'/214'],['可见英文',model.metrics.languageViolationCount]];document.querySelector('#metrics').innerHTML=metrics.map(([k,v])=>'<div class="metric"><span>'+k+'</span><strong>'+v+'</strong></div>').join('');
const baseMeta={'C-00':['顾客端基础外壳','移动首页：城市、消息、搜索、服务卡片、保障与四项主导航。','base','mobile'],'W-00':['师傅端基础外壳','移动工作台：身份、城市、任务大厅与五项主导航。','entry','mobile'],'A-00':['管理后台基础外壳','桌面运营总览：城市范围、指标、工作台入口与受控边界。','result','']};document.querySelector('#baseFrames').innerHTML=baseIds.map(id=>{const c=model.carriers.find(x=>x.carrierId===id);const m=baseMeta[id];const e=(c.baseFrame.edgeEvidence||[]).find(x=>x.stage===m[2])||c.baseFrame.edgeEvidence[0];return '<article class="base-card '+m[3]+'"><a class="preview" href="'+root+esc(e.file)+'"><img alt="'+esc(m[0])+'" src="'+root+esc(e.file)+'"></a><div class="copy"><span class="status '+esc(c.baseFrame.status)+'">'+esc(statusName[c.baseFrame.status])+'</span><h3>'+esc(m[0])+'</h3><p>'+esc(m[1])+'</p></div></article>'}).join('');
document.querySelector('#batchSlices').innerHTML=b0.map(x=>'<article class="slice-card"><div class="slice-title"><div><h3>'+esc(x.sliceId)+'</h3><small>'+esc(terminalName[x.terminal])+' · '+esc(x.carrierName)+'</small></div><span class="status '+esc(x.status)+'">'+esc(statusName[x.status])+'</span></div><div class="evidence-grid">'+(x.edgeEvidence||[]).map(e=>'<a class="evidence '+(x.terminal==='admin'?'':'mobile')+'" href="'+root+esc(e.file)+'"><img alt="'+esc(e.label)+'" src="'+root+esc(e.file)+'"><span>'+esc(e.label)+'</span></a>').join('')+'</div></article>').join('');
