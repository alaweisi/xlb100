...
<section class="section"><div class="head"><h2>36 个业务画面总览</h2><p>顾客端、师傅端与运营端均为手机 App；点击画面可查看原尺寸 Edge 截图。未来 OA 是独立专业网页系统，大屏是独立实时数据展示系统，不并入当前运营 App。</p></div><div class="base-grid" id="baseFrames"></div></section>
...
const terminalHelp={customer:'顾客端移动业务画面',worker:'师傅端移动工作画面',admin:'运营端手机业务画面'};document.querySelector('#baseFrames').innerHTML=baseIds.map(id=>{const c=model.carriers.find(x=>x.carrierId===id);const e=(c.baseFrame.edgeEvidence||[]).find(x=>x.stage==='base')||c.baseFrame.edgeEvidence?.[0];const preview=e?'<a class="preview" href="'+root+esc(e.file)+'"><img alt="'+esc(c.carrierName)+'" src="'+root+esc(e.file)+'"></a>':'<div class="preview" style="display:grid;place-items:center;color:#8a6422">证据尚未齐备</div>';return '<article class="base-card mobile">'+preview+'<div class="copy"><span class="status '+esc(c.baseFrame.status)+'">'+esc(statusName[c.baseFrame.status])+'</span><h3>'+esc(id+' · '+c.name)+'</h3><p>'+esc(terminalHelp[c.terminal]+' · '+c.surface)+'</p></div></article>'}).join('');
