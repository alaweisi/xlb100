...
# Phase 9A historical gate: admin settlement operations console UI scope only.
# Customer/worker UI changes are outside this locked Phase 9A admin settlement gate.
$allowedSettlementUi = @(
...
if ($LASTEXITCODE -ne 0) {
  Write-Host "check-phase9a-no-customer-worker-ui: FAILED - git diff failed (is main branch available?)"
  exit 1
}
...
if ($violations.Count -gt 0) {
  Write-Host "check-phase9a-no-customer-worker-ui: FAILED - settlement admin UI files changed"
  $violations | ForEach-Object { Write-Host "  $_" }
  exit 1
}

Write-Host "check-phase9a-no-customer-worker-ui: passed (settlement admin UI domain only)"
