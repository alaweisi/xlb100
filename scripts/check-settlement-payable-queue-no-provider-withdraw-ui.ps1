...
if ($Text -match '(?i)provider|withdraw|wechat|alipay|wallet|bank_account|payment_channel|payment_instruction') {
  throw "Phase 8E must not implement provider split or withdrawal."
}

# Settlement-payable-queue gate only applies to settlement admin UI surfaces in this phase.
$UiScope = @(
...
$UiChanges = @(
  git -C $Root diff --name-only 921f297d8f5471f2a55f1bedc99a5e9dee396680 -- $UiScope 2>$null |
