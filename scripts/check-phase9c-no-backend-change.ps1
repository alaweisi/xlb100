...
$vs = $diff | Where-Object { $_ -match 'backend/src/' -and $_ -notmatch 'backend/src/governance/' -and $_ -ne 'backend/src/app.ts' }
if ($vs) { Write-Host "check-phase9c-no-backend-change: FAILED — $vs"; exit 1 }
