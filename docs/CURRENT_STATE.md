...
| Customer UI full-slice refactor P6 | GATE 6 PASSED — INTEGRATED | — | A6→B6→C6 visual QA evidence integrated serially for all 9 Customer carriers; initial A6 P1/P2 findings fixed at `0895c1c2` and reverified with 25 scenarios and zero undersized interactive targets; final P0/P1/P2 = 0/0/0 |
| Customer UI full-slice refactor P7 | CLOSED — CUSTOMER-ONLY | — | 44 execution slices meet the Customer Definition of Done; integrated with the real OTP session and server-side order list; Home 4/4, A6 1/1, B6 2/2, C6 5/5, unit/contract 241 files / 1,243 tests and DB/security 203 files / 645 tests pass; Customer design gates, typecheck, lint and build pass; one non-blocking P3 retained; no Worker/Admin visual reconstruction |
...
- **Global regression truth**: the Customer P7 integration on 2026-07-22 removed the previously recorded failures without raising the frozen hardcode baseline and preserved real authentication/API authority. The current repository regression is GREEN: unit/contract 241 files / 1,243 tests passed plus 1 retained todo; DB/security 203 files / 645 tests passed with 1 environment-conditional skip; typecheck, lint, build, Phase25 Gate 1A/1B and Customer Home/A6/B6/C6 browser regressions pass.
...
- **Compatibility decision**: Phase25 presentation `Campaign.discountRuleId` is deprecated compatibility data and must never be read or executed by Marketing, Pricing, Quote, Order or frontend amount paths.
