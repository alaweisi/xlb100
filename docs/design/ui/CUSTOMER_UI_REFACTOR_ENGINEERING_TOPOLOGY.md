...
| Gate 5 | **PASS（2026-07-22）** — 通知、我的、地址及城市作用域跨路由恢复可用；A5 → B5 → C5 无冲突合流，14 文件 / 76 项聚焦测试、生产构建、设计/QA 门禁和 3 项浏览器验收通过；底部五导航仍是唯一一级导航 |
| Gate 6 | **PASS（2026-07-22）** — A6 → B6 → C6 证据串行合流；9 页面载体均有 default/ready 与最高风险状态证据；A6 首轮发现的 1 个 P1 与 1 个 P2 已在 `0895c1c2` 修复并复验；最终 P0/P1/P2 = 0/0/0 |
| Gate 7 | **PASS（2026-07-22）** — 44 个执行切片满足 Definition of Done；Customer 相关单测、typecheck、lint、依赖构建、主页门禁及 A6/B6/C6 浏览器回归通过；保留 1 个不阻断 P3 |
...
Gate 5 已于 2026-07-22 在同一集成线完成 A5 → B5 → C5 串行合流与共享入口校正：

- A5 `411d0304`：通知收件箱、归档、分页、已读/恢复和 409 冲突状态；合流提交 `bd7fc3cf`。
- B5 `23c80736`：账户资料与地址列表、新增、编辑、删除确认及保护性手机重录；合流提交 `83874987`。
- C5 `4e95156e`：统一 Customer deep-link 构造、城市作用域和目标页恢复；合流提交 `4ecc5956`。
- 集成线串行把通知→订单/客服、我的→通知/优惠券接入统一 deep-link helper；`App.tsx` 只在集成线修改。
- 三个槽位均从 Gate 4 基线创建、工作树 clean、页面文件交集为 0；没有 Worker、Admin、OA、Dashboard、后端、数据库或共享业务契约改动。
- Gate 5 聚合验证通过：14 个聚焦文件 / 76 项测试（含底部五导航唯一性）、Customer typecheck/lint、四项目依赖拓扑生产构建、Phase 25 设计门禁、9 路由 QA 基础设施，以及通知 2/2、个人中心/地址 1/1 真实浏览器验收。
- P6 只能从 Gate 5 关门提交创建；P6 负责 9 页面 × 3 视口的全量视觉证据与缺陷清零，不回退到任一 A5/B5/C5 支线。

Gate 6 与 Gate 7 已于 2026-07-22 在同一集成线完成最终串行收口：

- A6 `b0531017`（集成 `a88a0917`）：发现、服务目录、优惠券与下单链路证据；首轮如实判定 P1=1、P2=1。
- B6 `b3d352fa`（集成 `9fb0de9d`）：订单、支付、评价与售后证据；P0/P1/P2=0，登记 1 个不阻断 P3。
- C6 `368b5449`（集成 `9873ffde`）：客服、通知、我的与地址证据；P0/P1/P2=0。
- 集成线 `0895c1c2` 将下单表单、数量步进和恢复按钮提升到 44px 触控门禁，并把深链提示改为完成态中性语义；A6 强制复验后 25 个场景 undersized target = 0。
- P7 聚合结果：主页 4/4、A6 1/1、B6 2/2、C6 5/5、Customer 聚焦单测 17 文件 / 87 项、Customer typecheck/lint、四项目依赖构建、Phase25 设计与材质配方门禁全部通过。
- 44 个执行切片的统计不包含 `CUST-DS-001` 主页视觉真相本身；该项是所有切片的上游权威，因此清单共有 45 个勾选项但执行切片仍为 44。
- 发布前修复已在不抬高基线的前提下清零原 5 个失败：Customer `dimensionLiteral=35 ≤ 43`、`fontDeclaration=0 ≤ 1`、`numericZIndex=0`；仓库级回归 199/200 文件、621/622 测试通过，1 项按环境条件跳过；A6/B6/C6 浏览器回归 1/1、2/2、5/5 全部通过。详见 `CUSTOMER_UI_REFACTOR_P7_CLOSURE.md`。
- 本次未修改 Worker、Admin、OA、Dashboard、后端、数据库或共享业务契约；未 push、deploy、tag 或访问生产环境。
...
逐页替换可行，但发布底座必须选择已经具备正式登录、会话恢复和服务端订单列表的云候选；P7 只提供视觉与页面交互层，不反向覆盖业务真相。

| 顺序 | 替换面 | 必须保留的稳定能力 | 关门条件 |
| --- | --- | --- | --- |
| R0 | 共享 Token、服务资产、顾客端 Shell | 正式 OTP、会话过期处理、统一 API Client | 设计门禁、鉴权回归、构建通过 |
| R1 | 首页 | 服务端目录、诚实推荐/附近师傅空态 | 320/390/430 主页比较通过 |
| R2 | 服务目录、优惠券 | 官方类目、服务端优惠资格 | 搜索/空态/错误/深链通过 |
| R3 | 创建订单 | 服务端报价、地址、幂等与真实下单确认 | 4 步流程及失败恢复通过 |
| R4 | 订单 | 服务端 `listOrders`、支付/评价资格 | 换设备后仍能看到服务端订单 |
| R5 | 售后 | 服务端售后状态和证据 | 创建、处理中、确认状态通过 |
| R6 | 客服 | 工单、会话、重连和关闭状态 | ready/closed/recovery 通过 |
| R7 | 通知 | 服务端收件箱、409 冲突恢复、安全深链 | 已读/归档/恢复/跳转通过 |
| R8 | 我的、地址 | 正式身份、受保护地址编辑 | 查看/新增/编辑/删除通过 |
| R9 | 全量关门 | 上述全部能力 | 全量测试、浏览器 QA、云端 Smoke 全绿 |

每次只替换一个业务面并立即关门；任何一步失败都停在上一版可运行提交，不把未验收页面继续叠加到云候选。
