...
总账只登记当前代码、共享类型、状态机、API Client 和现有页面能够证明的业务。Figma 与历史 PNG 已退出权威链，不用于决定页面、状态、权限、金额或正式服务类目。
...
| 代码 | 事实源 |
| --- | --- |
| `ROUTE` | `apps/customer`、`apps/worker`、`apps/admin`、`apps/oa`、`apps/dashboard` 当前路由和页面。 |
...
盘点日期：2026-07-20。214 是顾客、师傅、后台三类交易与运营角色的唯一业务切片数，不包含同一业务在不同载体上的重复引用。OA 完整继承后台 98 条业务切片并使用同一事实源，仅增加总部身份、桌面布局和更高权限的承载义务；Dashboard 是这些事实的只读聚合观察面，不另建一套订单、派单、客服或售后数据。因此五端不会出现“同名指标、两套数据”的口径差异。
...
后台视口基线为 390×844 可安装移动 App；桌面批量工作台属于 OA。每个动作都继承 Admin 身份、城市范围、权限、幂等、版本冲突、确认和审计要求。后台不得把“准备、可支付、已排队、已导出、意向、标记付款”表述成真实资金执行。
