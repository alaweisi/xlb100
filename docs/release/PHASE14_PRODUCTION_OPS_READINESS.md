...
| PROD-OPS-007 | Monitoring and alerting | NOT RUN | SRE / Ops owner | Production evidence scaffold: `docs/release/PHASE14_PRODUCTION_MONITORING_EVIDENCE.md`; operator helper: `deploy/production/check-prod-monitoring.ps1`; still requires real production dashboard, alert, and notification evidence. | No production dashboard or alert evidence is recorded. | Blocks production because incidents would rely on manual log inspection. |
| PROD-OPS-008 | Payment/refund/reversal duplicate monitoring | NOT RUN | SRE / Finance ops owner | Production evidence scaffold: `docs/release/PHASE14_PRODUCTION_MONITORING_EVIDENCE.md`; still requires production/read-replica SQL output and duplicate alert evidence for `refund.approved` events and reversal ledger entries. | Duplicate prevention passed UAT, but production duplicate monitoring is not proven. | Blocks production financial operations visibility. |
| PROD-OPS-009 | Event handler lag monitoring | NOT RUN | SRE / Backend owner | Production evidence scaffold: `docs/release/PHASE14_PRODUCTION_MONITORING_EVIDENCE.md`; still requires pending event age query/alert for `event_outbox` rows where `event_type = 'refund.approved'`. | No production threshold, alert route, or escalation owner is recorded. | Blocks production because refund approval events could stall without detection. |
| PROD-OPS-010 | Replay/immutability release gate timing | NOT RUN | Release owner / Ledger owner | Procedure ready at `docs/release/PHASE14_RELEASE_WINDOW_GATE_TIMING.md`; final PASS requires isolated pre-cut and post-cut `npx pnpm preflight` logs with replay and immutability PASS evidence attached. | Validation has passed previously, including an isolated rerun after a concurrent transient replay mismatch, but required quiet release-window proof has not been run. | Blocks production because ledger replay and immutability must be proven at release time without concurrent DB writes. |
