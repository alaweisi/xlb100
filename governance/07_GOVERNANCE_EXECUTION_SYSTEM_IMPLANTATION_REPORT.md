...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-managed-worktree-boundaries.ps1 -Mode SelfTest
PASS — 100 negative fixtures
...
## 6. 2026-07-15 Skill 审核修订

- 删除 `xlb-context-map/reference.md` 与 `xlb-phase-boundary/reference.md`。两者把 Phase 8C、migration `014` 与售后占位状态写成当前事实，已不具备安全用途；
- 保留 `xlb-session-sync`、`xlb-context-map`、`xlb-current-vs-target`、`xlb-phase-boundary` 四个职责不同的 Skill，改为 session sync 必跑、其余按任务触发，禁止机械叠加；
- 固定 `refs/heads/main` 的解析 commit 为 committed control authority；canonical 目录当前 checkout 与 Bootstrap branch 只可提供实现/候选证据；
- WorkUnit Gate 在启动时把 `refs/heads/main` 解析为 immutable control commit，避免 branch switch 导致 registry/Manifest/lease/queue 事实漂移；
- 按 P-18 统一 Lock 顺序：pre-merge evidence → main merge → main 复验 → Lock metadata commit → canonical tag，并在 merge 与 Lock fact 写入前断言 `main`；
- 修正 BOM-less UTF-8 `CURRENT_STATE` 快照读取；明确 L3 为“施工前 Human 批准、进入 Verified 前补齐完整测试和两级审计 evidence”。
- 统一 Redis 隔离为独立 instance/namespace/volume/port，并把 Gap trace 从固定 G-01～G-16 改为“已登记稳定 ID 可追加、不可复用改号”。

上述修订不构成独立审计，也不改变 `BOOTSTRAP / NOT_ENABLED`。候选仍须以最终 immutable commit 重新执行 Repository Gate 与完整负向 SelfTest，并由未参与写入的 reviewer 独立审计。
