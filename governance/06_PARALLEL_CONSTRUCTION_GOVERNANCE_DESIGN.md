...
- `STALE`：base/contract/dependency/evidence revision 变化；
...
- `G:\xlb100` 是唯一 canonical control/integration/main/Lock root；只有 validation manifest 登记的工棚可用于非业务隔离验证；
...
本文件是正式工程施工宪法的并行执行模型，但不是任何具体业务 Train 的施工许可证。本轮 candidate 可修改治理执行文件、受管 validation worktree 和 disposable local isolation resources；不得修改 Phase 状态、业务 runtime、migration、hosted CI、main、tag 或 Lock，也不授权 production、Provider、push、deploy、replay、backfill 或 purge。执行系统须经 candidate commit、独立审计与 Human 确认后才能从 `BOOTSTRAP / NOT_ENABLED` 切换；任何实际 Train 仍必须由唯一 Human Owner通过通俗 Train Charter 明确批准。
