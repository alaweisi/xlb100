...
describe.skipIf(!runDb)("fulfillmentNoOrderPaymentMutation", { timeout: 30000 }, () => {
  ...
  it("keeps order and payment paid after start and complete", async () => {
    ...
    const [rows] = await getMysqlPool().query<(RowDataPacket & { order_status: string; payment_status: string })[]>(
      `SELECT o.status AS order_status, p.status AS payment_status
       FROM orders o JOIN payment_orders p ON p.order_id = o.order_id AND p.city_code = o.city_code
       WHERE o.order_id = ?`,
      [orderId],
    );
    expect(rows[0]).toMatchObject({ order_status: "paid", payment_status: "paid" });
    await app.close();
  });
});
