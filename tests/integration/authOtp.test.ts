...
describe.skipIf(!runDb)("auth OTP integration", { timeout: 20000 }, () => {
  ...
  it("does not disclose or authenticate a legacy worker from a masked-only phone identity", async () => {
    const app = await buildApp();
    ...
    try {
      ...
      const debug = await app.inject({
        method: "GET",
        url: `/api/auth/worker/debug-code?phone=${encodeURIComponent(phoneValue)}`,
      });
      expect(debug.statusCode).toBe(200);
      ...
      const login = await app.inject({
        ...
        payload: { phone: phoneValue, code: debug.json().code },
      });
  ...
  it("does not disclose or authenticate an inactive worker identity", async () => {
    const app = await buildApp();
    const pool = getMysqlPool();
    const phoneValue = "13877775678";
    const id = "worker-phone-inactive";
    try {
      await pool.query(
        `INSERT INTO worker_profiles (worker_id, display_name, phone_masked, phone_hash, status)
         VALUES (?, 'Inactive Worker', '138****5678', ?, 'inactive')
         ON DUPLICATE KEY UPDATE phone_hash = VALUES(phone_hash), status = 'inactive'`,
        [id, hashPhoneIdentity(phoneValue)],
      );
  ...
  it("does not disclose or authenticate an unknown admin identity", async () => {
    const app = await buildApp();
    const username = `missing_admin_${Date.now().toString(36)}`;
    try {
      const response = await app.inject({
        method: "POST",
  ...
  it("continues to issue and authenticate an existing admin identity", async () => {
    const app = await buildApp();
    const pool = getMysqlPool();
    const suffix = Date.now().toString(36);
    const id = `admin-otp-${suffix}`;
    const username = `admin_otp_${suffix}`;
    try {
      await pool.query(
        `INSERT INTO admin_users (id, username, role, city_scopes_json)
         VALUES (?, ?, 'operator', JSON_ARRAY('hangzhou'))`,
        [id, username],
      );
