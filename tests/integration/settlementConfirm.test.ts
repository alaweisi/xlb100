...
describe.skipIf(process.env.XLB_SKIP_DB_TESTS === "1")("settlement confirmation", { timeout: 60000 }, () => {
  it("confirms a prepared batch and all snapshot items", () => withSettlementTestLock(async () => {
    ...
    try {
      ...
      const missingIdentity = await app.inject({ method: "POST", url: `/api/internal/settlement/batches/${batch.settlementBatchId}/confirm`, headers: { "x-xlb-app-type": "admin", "x-xlb-role": "operator", "x-xlb-city-code": "hangzhou" }, payload: {} });
