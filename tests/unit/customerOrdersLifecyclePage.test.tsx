...
function renderOrders(api: ReturnType<typeof createApi>, orderIds: string[]) {
  return render(
    <ThemeProvider>
      <CustomerOrdersPage api={api} cityCode="hangzhou" orderIds={orderIds} />
    </ThemeProvider>,
  );
}
...
describe("CustomerOrdersPage complete lifecycle", () => {
  ...
  it("keeps successfully loaded orders visible when another order fails", async () => {
    const visible = order("paid");
    const api = createApi({ [visible.orderId]: visible });
    api.getOrder.mockImplementation(async (orderId: string) => {
      if (orderId === "order-missing") throw new Error("network");
      return { order: visible };
    });
    renderOrders(api, [visible.orderId, "order-missing"]);

    expect(await screen.findByText("部分订单暂未加载")).not.toBeNull();
    expect(screen.getByText("2小时日常保洁")).not.toBeNull();
  ...
  it("does not claim payment success when the command result is uncertain", async () => {
    ...
    const api = createApi({ [current.orderId]: current });
    api.mockPaySuccess.mockRejectedValueOnce({ kind: "timeout" });
    renderOrders(api, [current.orderId]);
  ...
  it("shows payment success only after the payment command and refreshed order both succeed", async () => {
    const current = order("service_completed");
    ...
    expect(api.mockPaySuccess).toHaveBeenCalledTimes(1);
    expect(await screen.findByText("支付成功，订单已完成。")).not.toBeNull();
    expect(screen.getAllByText("已完成").length).toBeGreaterThan(0);
  });
