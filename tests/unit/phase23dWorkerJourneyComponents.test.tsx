...
describe("Phase 23D Worker journey page components", () => {
  it("routes an eligible queued task to the real accept callback and locks it while busy", () => {
    ...
    expect((screen.getByRole("button", { name: "正在接单…" }) as HTMLButtonElement).disabled).toBe(true);
  });
  ...
  it("enforces accepted-to-start and in-progress-to-complete lifecycle actions", () => {
    ...
    const props = {
      ...
      onBack: vi.fn(), onRetry: vi.fn(), onStart, onComplete, onRefreshEvidence: vi.fn(), onUploadEvidence: vi.fn(),
    };
    const { rerender } = render(<TaskDetailPage {...props} />);

    const start = screen.getByRole("button", { name: "开始服务" }) as HTMLButtonElement;
    const complete = screen.getByRole("button", { name: "完成服务" }) as HTMLButtonElement;
    ...
    const enabledComplete = screen.getByRole("button", { name: "完成服务" }) as HTMLButtonElement;
