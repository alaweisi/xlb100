...
vi.mock("@xlb/api-client", () => ({
  ...
  customerApi: { forClient: vi.fn(() => ({ getCatalog: mocks.getCatalog })) },
}));
...
describe("B0 顾客身份 Gate", () => {
  ...
  it("保留原目标并通过真实验证码动作建立持久会话", async () => {
    ...
    expect(await screen.findByText("顾客身份验证")).toBeTruthy();
    expect(screen.getByText("验证完成后返回：订单")).toBeTruthy();
    ...
    await waitFor(() => expect(screen.queryByText("顾客身份验证")).toBeNull());
  });
});
