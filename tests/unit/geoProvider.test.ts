...
describe("Phase 20 local geo provider",()=>{
  ...
  it("models a retryable provider outage without an external call",async()=>{const unavailable=new LocalMockGeoProvider({transport:"transient_failure"});await expect(unavailable.geocode("hangzhou","address")).rejects.toMatchObject({code:"SIMULATED_TRANSIENT_FAILURE",retryable:true,externalProviderExecuted:false});});
});
