...
test("B6 orders visual QA covers server-list ready, payment entry and review composition", async ({ page, context }) => {
  const consoleErrors = collectConsoleErrors(page);
  await seedCustomerSession(page, ["order-paid-b6", "order-payable-b6"]);
  await page.route(/\/api\/customer\/orders(?:\?.*)?$/, (route) => route.fulfill({
    status: 200,
    contentType: "application/json; charset=utf-8",
    body: JSON.stringify({ ok: true, orders: [paidOrder, payableOrder], nextCursor: null }),
  }));
  ...
  await writeFile(join(process.cwd(), evidenceRoot, "orders-automated.report.json"), `${JSON.stringify({
    ...
    viewports,
    states: ["server-list-ready", "payment-entry", "review-compose"],
    checks: {
      authoritativeStatusLabels: true,
      onePrimaryNextAction: true,
      serverOrderListVisible: true,
      ...
      expectedFixtureNetworkErrors: 0,
