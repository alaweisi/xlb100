...
describe.skipIf(!runDb)("fulfillmentCityWorkerScoped", { timeout: 30000 }, () => {
  ...
  it("rejects wrong worker, city, missing city, and non-worker role", async () => {
    ...
    expect((await app.inject({ method: "POST", url, headers: { "x-xlb-app-type": "worker", "x-xlb-role": "worker", "x-xlb-user-id": "worker-demo-hangzhou" }, payload: {} })).statusCode).toBe(400);
    expect((await app.inject({ method: "POST", url, headers: { ...workerHangzhouHeaders, "x-xlb-app-type": "customer", "x-xlb-role": "customer" }, payload: {} })).statusCode).toBe(403);
