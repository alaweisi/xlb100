...
const customerPort = Number(process.env.CUSTOMER_AFTERSALE_QA_APP_PORT ?? "5392");
const customerUrl = `http://127.0.0.1:${customerPort}`;

export default defineConfig({
  testDir: "./tests/e2e/customer-ui",
  testMatch: "customerAftersaleEvidence.spec.ts",
  outputDir: "./test-results/customer-aftersale",
  timeout: 60_000,
  fullyParallel: false,
  workers: 1,
  reporter: [["list"]],
