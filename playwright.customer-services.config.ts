...
const customerPort = Number(process.env.CUSTOMER_SERVICES_QA_APP_PORT ?? "5387");
const backendUrl = `http://127.0.0.1:${backendPort}`;
const customerUrl = `http://127.0.0.1:${customerPort}`;

export default defineConfig({
  testDir: "./tests/e2e/customer-ui",
  testMatch: "customerServicesEvidence.spec.ts",
  outputDir: "./test-results/customer-services",
  timeout: 90_000,
  fullyParallel: false,
  workers: 1,
  reporter: [["list"]],
